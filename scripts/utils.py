...
def plot_roc_curve(y_true, y_score, axis, bootstrapping=1000, drop_intermediate=False, fontdict={}, name='ROC', color='b', show_wp=True, show_label=True):
    # ----------- Bootstrapping ------------
    ...
    axis.plot(fprs, tprs, color=color, label=rf"{name} (AUC = {auc_val:.2f} $\pm$ {std_auc:.2f})",
                lw=2, alpha=.8)
    label = r'$\pm$ 1 std. dev.' if show_label else ''
    axis.fill_between(mean_fpr, tprs_lower, tprs_upper, color='grey', alpha=.2, label=label) # r'$\pm$ 1 std. dev.'
    if show_wp:
